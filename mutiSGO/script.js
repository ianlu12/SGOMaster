...
function autoFight(limitHp,limitPower,floor)
{
    ...
    //完成休息  多種樣態
    var getStatusLength = document.getElementsByClassName('css-1riv80w')[0].childNodes[1].childNodes.length;
    if (getStatusLength == 3) {
        if (document.getElementsByClassName('css-1riv80w')[0].childNodes[1].childNodes[2].data == '（可完成）') {
            document.getElementsByClassName('css-1myfyhp')[0].childNodes[3].click();
            status = "完成休息(有狀態)";
            _log(status);
            return false;
        }
    } else if (getStatusLength == 2)
    {
    ...
    //自動戰鬥、自動休息
    if (power > limitHp && hp > limitPower && document.getElementsByClassName('css-1riv80w')[0].childNodes[1].childNodes[0].data == '閒置')
    {
